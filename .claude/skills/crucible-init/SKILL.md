...
> **참고**: `skills/`와 `agents/`는 `./setup --host claude`가 `~/.claude`에 전역 설치합니다. 이 단계에서는 프로젝트 로컬 구조만 생성합니다.

```
.claude/
├── hooks/               # ~/.claude/crucible-hooks/ 에서 복사
├── memory/
│   ├── decisions/       # ADR 저장소
│   ├── specs/           # 피처 사양
│   └── session-log/     # 세션 복구용 로그
├── gates/               # ~/.claude/crucible-gates/ 에서 복사
└── protocols/           # MCP + 도구 정책
```

hooks와 gates는 전역 템플릿에서 복사합니다:
- `~/.claude/crucible-hooks/*.sh` → `.claude/hooks/`
- `~/.claude/crucible-gates/*.md` → `.claude/gates/`

전역 템플릿이 없으면 (`./setup --host claude`를 아직 실행하지 않은 경우) 사용자에게 먼저 설치를 안내합니다.
