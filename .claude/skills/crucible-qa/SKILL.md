...
> 공개 저장소에는 `bin/`과 `node_modules/`를 커밋하지 않습니다. 브라우저 도구가 필요하면 repo root에서 `./install.sh claude --with-browser` 또는 `./install.sh codex --with-browser`를 실행해 생성합니다.
...
> 참고: 현재 프로젝트처럼 `ai-agent` 또는 `documentation-first` 상태에서는 이 스킬이 기본 경로가 아닙니다. web-app 구현이 실제로 존재할 때만 실행합니다.
